...
- 新增 `rag/safety.py`，实现 `SafetyChecker` 和 `SafetyDecision`
- 支持识别“忽略以上规则”“系统提示词”“jailbreak”等提示词注入风险
- 支持识别 localhost、127.0.0.1、私有 IP、链路本地地址等危险 URL
- 更新 `rag/service.py`，在检索前执行安全检查
- 更新 `tests/test_api.py`，覆盖 API 安全拒答
- 新增 `tests/test_safety.py`，覆盖提示词注入、危险 URL、正常问题和服务级安全拒答
- 更新 README，记录安全校验能力
